...
// ── Raw DB row types (snake_case matches SQL aliases) ──────────────
...
export async function getDemandTrends(
  days: number = 30,
): Promise<DemandTrendRow[]> {
  try {
    const { rows } = await db.query<DemandTrendRow>(
      `
      SELECT
        product_name,
        category,
...
export async function getSupplyTrends(): Promise<StockTrendRow[]> {
  try {
    const { rows } = await db.query<StockTrendRow>(
      `
      SELECT
...
export function generateForecastAnalysis(
  demandTrends: DemandTrendRow[],
  supplyTrends: StockTrendRow[],
  ...
  if (demandTrends.length > 0) {
    ...
    parts.push(
      `📈 Top demanded products (last 30 days): ${topDemand.map((d) => `${d.product_name} (${d.total_demand} orders)`).join(', ')}.`,
    );
  ...
  if (supplyTrends.length > 0) {
    ...
    parts.push(
      `📦 Most stocked products: ${topSupply.map((s) => `${s.product_name} (${s.supplier_count} suppliers, avg ৳${s.avg_price})`).join(', ')}.`,
    );
  ...
  if (productHint) {
    const relatedDemand = demandTrends.filter(
      (d) => d.product_name.toLowerCase().includes(productHint.toLowerCase()),
    );
    const relatedSupply = supplyTrends.filter(
      (s) => s.product_name.toLowerCase().includes(productHint.toLowerCase()),
    );
