...
import { ChatIntent, classifyIntent, parseProductIntent, parseBulkOrderItems, findSupplierByName, getSupplierCatalog, searchSuppliers, searchAllSuppliers, generateGreetingResponse, generateSearchResponse, generateUnknownResponse, } from '../services/assistantService.js';
 ...
 */
export const placeChatbotOrderHandler = asyncHandler(async (req, res) => {
    const shopOwnerId = req.userId;
    const { stockholderId, items } = req.body;
    ...
    try {
        ...
        const ownerRes = await client.query(`SELECT latitude, longitude FROM public.users WHERE id = $1`, [
            shopOwnerId,
        ]);
        ...
        for (const item of items) {
            ...
            const productRes = await client.query(`SELECT custom_product_name, category, price_per_unit, unit
           FROM public.stockholder_inventory
           WHERE stockholder_id = $1
             AND LOWER(custom_product_name) = LOWER($2)
           LIMIT 1`, [stockholderId, rawName]);
            ...
            const targetPrice = match?.price_per_unit != null ? Number(match.price_per_unit) : null;
            const demandRes = await client.query(`INSERT INTO public.demands
 ...
 */
export const assistantChatHandler = asyncHandler(async (req, res) => {
    const { message, shopLat, shopLng } = req.body;
    // ── Validate input ───────────────────────────────────────────
    ...
    try {
        ...
        if (multiItems) {
            const resolved = await Promise.all(multiItems.map(async (entry) => {
            ...
            const estimatedTotal = resolved.reduce((sum, r) => sum + (r.match ? r.match.pricePerUnit * r.quantity : 0), 0);
        ...
        switch (intent) {
            ...
            case ChatIntent.PRODUCT_SEARCH: {
                ...
                if (supplier) {
                    const catalog = await getSupplierCatalog(supplier.stockholderId, lat, lng);
                    if (catalog.length > 0) {
