...
import { submitReview, getSupplierRating, getInventoryRating, listInventoryReviews, resolveOrderForReview, } from '../services/reviewService.js';
...
export const getSupplierRatingHandler = asyncHandler(async (req, res) => {
...
export const getInventoryRatingHandler = asyncHandler(async (req, res) => {
...
export const listInventoryReviewsHandler = asyncHandler(async (req, res) => {
    const inventoryId = String(req.params.inventoryId);
    if (!inventoryId || !UUID_RE.test(inventoryId)) {
        res.status(400).json({ success: false, error: 'Valid inventoryId is required' });
        return;
...
export const resolveOrderHandler = asyncHandler(async (req, res) => {
    const shopOwnerId = req.userId;
