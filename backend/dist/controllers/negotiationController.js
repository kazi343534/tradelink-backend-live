...
import { initiateNegotiation, listShopOwnerNegotiations, listSupplierNegotiations, counterNegotiation, respondToNegotiation, sendNegotiationMessage, getNegotiationThread, finalizeNegotiation, } from '../services/negotiationService.js';
/**
 ...
 */
export const initiateNegotiationHandler = asyncHandler(async (req, res) => {
    const shopOwnerId = req.userId;
    const { stockId, quantity, proposedPrice, message } = req.body;
 ...
 */
export const getShopOwnerNegotiationsHandler = asyncHandler(async (req, res) => {
    const shopOwnerId = req.userId;
 ...
 */
export const getSupplierNegotiationsHandler = asyncHandler(async (req, res) => {
 ...
 */
export const counterNegotiationHandler = asyncHandler(async (req, res) => {
    const stockholderId = req.userId;
    const { negotiationId, proposedPrice, counterMessage } = req.body;
 ...
 */
export const respondToNegotiationHandler = asyncHandler(async (req, res) => {
    const shopOwnerId = req.userId;
 ...
 */
export const sendNegotiationMessageHandler = asyncHandler(async (req, res) => {
    ...
    const { negotiationId, message, offeredPrice } = req.body;
    if (!negotiationId) {
    ...
    const negotiation = await sendNegotiationMessage({
        ...
        offeredPrice: offeredPrice != null && Number(offeredPrice) > 0
            ? Number(offeredPrice)
            : null,
    });
    res.json({ success: true, data: negotiation });
 ...
 */
export const getNegotiationMessagesHandler = asyncHandler(async (req, res) => {
 ...
 */
export const getSupplierNegotiationsByIdHandler = asyncHandler(async (req, res) => {
    ...
    const negotiations = await listSupplierNegotiations(String(req.params.stockholderId));
    res.json({ success: true, data: negotiations });
});
/**
 ...
 */
export const finalizeNegotiationHandler = asyncHandler(async (req, res) => {
    const requesterId = req.userId;
