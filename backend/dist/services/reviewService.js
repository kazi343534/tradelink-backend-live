 ...
 */
export async function submitReview(shopOwnerId, input) {
    ...
    try {
        ...
        if (!supplierId && orderId) {
            const { rows } = await client.query('SELECT supplier_id, product_name FROM public.orders WHERE id = $1', [orderId]);
            if (rows.length > 0) {
        ...
        if (!inventoryId && orderId && supplierId) {
            const { rows: orderRows } = await client.query('SELECT product_name FROM public.orders WHERE id = $1', [orderId]);
           ...
           LIMIT 1`, [supplierId, orderRows[0].product_name]);
        ...
        if (orderId) {
            result = await client.query(`INSERT INTO public.ratings (order_id, shop_owner_id, supplier_id, inventory_id, rating, review)
         VALUES ($1, $2, $3, $4, $5, $6)
         ...
         RETURNING id, order_id, shop_owner_id, supplier_id, inventory_id, rating, review, created_at`, [orderId, shopOwnerId, supplierId, inventoryId, rating, comment ?? null]);
        }
        else {
            result = await client.query(`INSERT INTO public.ratings (order_id, shop_owner_id, supplier_id, inventory_id, rating, review)
         VALUES (NULL, $1, $2, $3, $4, $5)
         RETURNING id, order_id, shop_owner_id, supplier_id, inventory_id, rating, review, created_at`, [shopOwnerId, supplierId, inventoryId, rating, comment ?? null]);
        }
 ...
 */
export async function resolveOrderForReview(shopOwnerId, productName) {
    const { rows } = await db.query(`SELECT o.id AS order_id, o.supplier_id,
     ...
     LIMIT 1`, [shopOwnerId, `%${productName}%`]);
    if (rows.length === 0)
        return null;
    return {
 ...
 */
export async function getSupplierRating(supplierId) {
 ...
 */
export async function getInventoryRating(inventoryId) {
    const { rows } = await db.query(`SELECT COALESCE(rating, 5.0) AS rating, COALESCE(review_count, 0) AS review_count
     FROM public.stockholder_inventory
     WHERE id = $1`, [inventoryId]);
    return {
        rating: Number(rows[0]?.rating ?? 5.0),
        reviewCount: rows[0]?.review_count ?? 0,
    };
}
export async function listInventoryReviews(inventoryId, limit = 20) {
    const { rows } = await db.query(`SELECT r.id, r.rating, r.review, r.created_at,
