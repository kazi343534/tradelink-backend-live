 ...
 */
export async function createDirectOrder(shopOwnerId, payload) {
    const client = await db.connect();
    try {
        await client.query('BEGIN');
        // 1. Lock and validate the stock item
        const { rows: stockRows } = await client.query(`SELECT id, stockholder_id, custom_product_name, price_per_unit, quantity_available, unit
       FROM stockholder_inventory
       WHERE id = $1 AND is_available = true
       FOR UPDATE`, [payload.stockId]);
        const stock = stockRows[0];
        if (!stock)
            throw httpError('Product not found or unavailable', 404);
        if (stock.quantity_available < payload.quantity) {
            throw httpError(`Insufficient stock. Only ${stock.quantity_available} ${stock.unit} available.`, 409);
        }
...
//# sourceMappingURL=directOrderService.js.map